...
from asi_core.camera.ocam import OcamModel
from asi_core.camera.mobotix import extract_mobotix_meta_data
from asi_core.config import config_loader
from asi_core.image.transform import (
    mask_image_batch,
    resize_image_batch,
    asi_undistortion,
    get_zenith_cropping,
...
from asi_core.utils.datetime_handling import timezone_ISO8601_to_pytz_posix
from asi_core.utils.filesystem import assemble_path, fstring_to_re
...
class AllSkyImager:
    ...
    def __init__(self, camera_data, camera_id=None, image_path=None, tfms=None):
        ...
        self.img_timezone = pytz.timezone(timezone_ISO8601_to_pytz_posix(camera_data['timezone']))
    ...
    def from_config(cls):
        ...
        :return: list of AllSkyImager instances
        """
        cfg_vals = config_loader.get('Camera')
    ...
    def get_azimuth_elevation(self, ocam_model, min_ele_mask, external_orientation):
        ...
        az, ele, _ = geometry.cartesian_to_spherical(image_coord_yxz[:, 2], image_coord_yxz[:, 1], image_coord_yxz[:, 3])
        ...
        if not self.img_paths_buffer.get('prev_date') == present_day:
            ...
            if np.any(after_dismantle):
                ...
                logging.warning(f'Timestamps ({first_reject}...{last_reject}) are after the end date of recording '
                                f'({self.end_recording}) for the current camera configuration. Ignoring images.')
...
def load_camera_data(camera_data_file=None, camera_data_dir=None, camera_name=None, timestamp=None):
    ...
    if camera_data_file is None:
        ...
        camera_data_dir = ifnone(camera_data_dir, root / 'data' / 'camera_data')
