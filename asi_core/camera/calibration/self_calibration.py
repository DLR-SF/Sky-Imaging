...
from asi_core.utils import geometry
from asi_core.camera.sky_imager import AllSkyImager
...
class Calibration:
    ...
    def optimize_eor_ior(self, orb_observations, orientation_init=None, ocam_init=None, test_various_azimuths=True):
        ...
        for azimuth_init in azimuth_inits:
            ...
            logging.debug(f'Optimization termininated with message {res.message}')
    ...
    def angles_to_pixels(orb_observations, ocam, eor):
        ...
        x, y, z = geometry.spherical_to_cartesian(np.deg2rad(orb_observations['azimuth']),
    ...
    def angles_pixels_to_vector_deviation(orb_observations, ocam, eor, ignore_outliers_above_percentile=None,
        ...
        r = np.ones(np.shape(orb_observations['azimuth']))
        # 2. transform azimuth and elevation angle (provided as dataframe over all timestamps) into image coordinates
        expected_3d_x, expected_3d_y, expected_3d_z = geometry.spherical_to_cartesian(
        ...
        orb_observations['found_azimuth'], orb_observations['found_elevation'], _ = geometry.cartesian_to_spherical(
...
def plot_orb_positions(orb_observations_path, background_image_path='',
    ...
    plt.text(0.98, 0.02, f'RMSE [pxl]: {rmse:.2f}\nRMSE [pxl] w/o {100-outliers_above_percentile_ignored_in_calib}% outliers: {rmse_cleaned:.2f}',
             transform=plt.gca().transAxes, ha='right', va='bottom', fontsize=8, color='green')
...
def get_background_img_and_plot(camera, orb_obs_file, exp_time, image_file_name,
    ...
    orb_data.sort_values(by='timestamp', inplace=True)
    background_image_path = []
    for img_timestamp in pd.date_range(orb_data.timestamp.iloc[0].astimezone(camera.img_timezone),
...
class CenterDetector:
    ...
    def find_best_threshold(self, img, min_contour_area_ratio=0.1):
        ...
        for threshold_value in range(1, self.max_threshold_value, 1):
            ...
            for cnt in contours:
                if cv2.contourArea(cnt) >= min_contour_area and geometry.is_circle_contour(cnt)[0]:
