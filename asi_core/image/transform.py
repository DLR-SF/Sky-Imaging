...
def resize_image(image, resize):
    ...
    resized_image = np.asarray(Image.fromarray(image.squeeze()).resize(resize, interpolation))
...
def asi_index_cropping_inverse(cropped_asi, original_size, crop_x, crop_y, channel_first=False):
    """
    Restores a cropped image to its original dimensions by placing the cropped
    image into a blank image of the original size.

    :param cropped_asi: The cropped image.
    :param original_size: The shape of the original image (tuple).
    :param crop_x: The x-coordinates (rows) used for cropping.
    :param crop_y: The y-coordinates (columns) used for cropping.
    :param channel_first: Whether the color channel is the first dimension.
    :return: The restored image with original dimensions.
    """
    # Create a blank image of the original size
    restored = np.zeros(original_size, dtype=cropped_asi.dtype)

    if channel_first:
        if len(original_size) == 3:
            restored[:, crop_x, crop_y] = cropped_asi
