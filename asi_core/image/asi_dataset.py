...
"""
This module provides functions to create ASI (All Sky Imager) images datasets, 
e.g., for machine learning applications.
"""
...
from asi_core.camera.sky_imager import load_camera_data, AllSkyImager
from asi_core.camera.mobotix import get_mobotix_meta_data

Q25_LEFT_CENTRATION_THRESHOLD = 200
Q25_RIGHT_CENTRATION_THRESHOLD = 400


def load_asi_image_list(csv_files, asi_root=None, col_timestamp='timestamp', col_rel_path='rel_path',
                  col_filename='file_name'):
    """Loads list of ASI images from csv files. If asi_root is passed, the absolute path of each 
    ASI image is appended to the resulting dataframe.
...
def read_asi_image_meta_data(filename, is_mobotix=True, name_convention='dlr', tz='UTC+0100'):
    """Extracts meta data of an ASI image from its name.

    :param filename: file path of ASI image.
...
def check_asi_image_list(asi_image_list, tz=1, limit_exp_time=None, name_convention='dlr', n_workers=0):
    """Checks a list of ASI image files for corruption and returns dataframe with additional data.

    :param asi_image_list: list of ASI image files.
    ...
    checked_list = parallel(check_asi_image, asi_image_list, tz=tz, limit_exp_time=limit_exp_time, name_convention=name_convention,
                            n_workers=n_workers)
    df_asi = pd.DataFrame(checked_list)
    return df_asi


def check_asi_image(filename, tz="UTC+0100", is_mobotix=True, limit_exp_time=None, name_convention='dlr'):
    """
    Checks a single ASI image file for corruption and extracts metadata.
    ...
    asi_dict = {}
    try:
        meta_data = read_asi_image_meta_data(filename, tz=tz, is_mobotix=is_mobotix, name_convention=name_convention)
...
def load_transform_save_asi_image(rel_path, all_sky_imager, source_dir, target_dir):
    """Loads, transforms and saves transformed ASI image.
...
def create_asi_image_list(asi_root, do_check=False, name_convention='dlr', csv_file=None, n_workers=0):
    """Gets all ASI images within asi_root and save the list to csv.
    ...
    if do_check:
        df_asi = check_asi_image_list(all_asi, name_convention=name_convention, n_workers=n_workers)
...
def create_asi_dataset(asi_series, source_dir, target_dir, camera_data_dir, n_workers=0, asi_tfms=None):
    """Creates an ASI image dataset from all passed filenames in target_dir.
    ...
    for cam_id, cam_dict in camera_data.items():
        ...
        fails = parallel(load_transform_save_asi_image, part_asi_series, all_sky_imager=all_sky_imager, source_dir=source_dir,
                         target_dir=target_dir, n_workers=n_workers, progress=True)
...
def read_asi_dataset(csv_file, img_dir=None, asi_path_col='rel_path', drop_asi_filepath=True, filter_dates=None):
    """
    Reads an ASI image dataset from a CSV file and optionally filters by date.
...
def merge_meteo_and_asi_data(df_meteo, df_asi, temporal_resolution='30s', max_delta_t=15, parameters_to_cast=None):
    """
    Merges meteorological data with ASI images data based on timestamps.

    :param df_meteo: Pandas DataFrame containing meteorological data indexed by timestamp.
    :param df_asi: Pandas DataFrame containing ASI images metadata indexed by timestamp.
    ...
    """
    df_asi_mapped = map_asi_image_to_timestamps(df_asi, round_to=temporal_resolution, max_delta_t=max_delta_t)
...
def map_asi_image_to_timestamps(df, round_to='60s', max_delta_t=10, valid_exp_times=None, max_delta_exp_time=10,
                          multi_exposure=False, inplace=False):
    """Maps ASI image acqusition time to a rounded timestamp.
...
def check_Q25_asi_image_cropping(rel_path_to_image, asi_root):
    """This function can be used to determine how images from the Q25 ASI have been cropped for custom
