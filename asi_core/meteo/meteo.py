...
from asi_core.utils.datetime_handling import get_temporal_resolution_from_timeseries
from asi_core.dlr.matlab_converters import map_data_columns, load_matlab_mesor_file, load_matlab_dni_classes, load_matlab_linke_turbidity_per_day
from asi_core.config.constants import PYTHON_LAT, PYTHON_LON, PYTHON_ALT, PYTHON_TZ, PYTHON_DT, PYTHON_PAMB
from asi_core.real_time import utils
...
class MeteoData(object):
    ...
    def compute_linke_turbidity(self, method_linke_turbidity='ineichen_dlr', solar_constant=1361.1, limit_sun_elevation=10,
    ...
    def compute_clear_sky_irradiance(self, method_clear_sky_irradiance='ineichen', method_linke_turbidity='ineichen_dlr',
                                     solar_constant=1364):
        """
        Computes the clear sky irradiance using `pvlib`.

        This method calculates the clear sky Global Horizontal Irradiance (GHI), Direct Normal Irradiance (DNI),
        and Diffuse Horizontal Irradiance (DHI) based on solar position, airmass, and Linke turbidity factors.
        It uses the specified clear sky irradiance model (currently supports the 'ineichen' model) and Linke
        turbidity method.

        The computed values are stored in the `self.data` DataFrame under the columns:
        - `'clear_sky_ghi'`: Clear sky GHI (W/m²)
        - `'clear_sky_dni'`: Clear sky DNI (W/m²)
        - `'clear_sky_dhi'`: Clear sky DHI (W/m²)

        Additionally, the method applies masking rules to adjust clear sky estimates:
        - If measured GHI exceeds 1.4 times the computed clear sky GHI, it replaces the clear sky GHI with measured GHI.
        - If measured DNI exceeds the computed clear sky DNI, it replaces the clear sky DNI with measured DNI.
        - Clear sky DHI is recomputed as the difference between GHI and DNI, adjusted by the solar elevation angle.

        :param method_clear_sky_irradiance: Clear sky model to use (currently only 'ineichen' is supported).
        :type method_clear_sky_irradiance: str
        :param method_linke_turbidity: Method for calculating Linke turbidity ('ineichen_dlr' is recommended).
        :type method_linke_turbidity: str
        :param solar_constant: Value of the solar constant (W/m²), default is 1364 W/m².
        :type solar_constant: float
        :returns: DataFrame with columns `'clear_sky_ghi'`, `'clear_sky_dni'`, `'clear_sky_dhi'`.
        :rtype: pandas.DataFrame
        :raises NotImplementedError: If an unsupported clear sky model is requested.
        """
        solar_pos = self.get_solar_pos()
        zenith = 90 - solar_pos['sun_el']
        ...
        if method_clear_sky_irradiance == 'ineichen':
            df_clear_sky = pvlib.clearsky.ineichen(apparent_zenith=zenith, altitude=self.altitude,
                                                   ...
                                                   dni_extra=solar_constant)
        else:
            raise NotImplementedError(f'Method {method_clear_sky_irradiance} not implemented.')
    ...
    def plot_distributions(self, columns=None, n_rows=2, df_ref=None, figsize=(20, 12), **kwargs):
        if columns is None:
            columns = list(self.data.columns)
        plot_data_distributions(self.data, columns=columns, n_rows=n_rows, df_ref=df_ref, figsize=figsize, **kwargs)
    ...
    @classmethod
    def from_csv(cls, csv_file, delimiter=',', header='infer', rename_columns={}, **kwargs):
        """Initialize MeteoData object from csv file with measurements.

        :param csv_file: csv file from which csv should be initialized
        :type csv_file: str
        :param delimiter: argument as used by pd.read_csv
        :type delimiter: str
        :param header: argument as used by pd.read_csv
        :type header: int or str or None
        :param rename_columns: falsy or mapping from column names in csv file to column names expected by cls
        :type rename_columns: dict
        :params kwargs: keyword arguments accepted by the cls init method
        :type kwargs: dict
        :return: cls instance
        """
        ...
        for k, v in rename_columns.items():
            if v == index_col:
                index_col = k
                break
        df_csv = pd.read_csv(csv_file, parse_dates=[index_col], index_col=index_col, delimiter=delimiter,
                             header=header)
        if rename_columns:
            df_csv.rename(columns=rename_columns, inplace=True)

        tz_detected = df_csv.index.tz
        if tz_detected is not None:
            kwargs['tz'] = tz_detected

        mode_freq = get_temporal_resolution_from_timeseries(df_csv)
        # Express temporal resolution in seconds
        temp_res = f'{mode_freq.total_seconds():.0f}s'
        return cls(data=df_csv, temporal_resolution=temp_res, **kwargs)

    @classmethod
    def from_pangaea_tab(cls, tab_file, delimiter=',', header='infer', rename_columns={}, **kwargs):
        """Initialize MeteoData object from csv file with measurements downloaded from Pangea.

        Tested so far with this dataset:
        Fabel, Yann; Blum, Niklas; Nouri, Bijan; Wilbert, Stefan; Gonzalez Rodriguez, Sergio; Zarzalejo, Luis (2025):
        SolarVision Almería: Meteorological measurements [dataset]. PANGAEA, https://doi.org/10.1594/PANGAEA.980085

        :param tab_file: tab-separated table file from which csv should be initialized
        :type tab_file: str
        :param delimiter: argument as used by pd.read_csv
        :type delimiter: str
        :param header: argument as used by pd.read_csv
        ...
        with open(tab_file, 'r', encoding='utf-8') as file:
            for i in range(header_lines):
                ...
                if 'Event(s):' in meta_line:
                    break

        meta_infos = [{'pattern': r'Event\(s\):\t(?P<name>\S*)\s', 'convert': None},
                      {'pattern': r'LATITUDE:\s(?P<latitude>\S*)\s', 'convert': float},
                      {'pattern': r'LONGITUDE:\s(?P<longitude>\S*)\s', 'convert': float},
                      {'pattern': r'ELEVATION:\s(?P<altitude>\S*)\s', 'convert': float}]

        for meta_info in meta_infos:
            meta_parsed = re.search(meta_info['pattern'], meta_line)
            if meta_parsed is None:
                logging.warning(f'Pattern not found in csv header {meta_info["pattern"]}. Meta info will remain unset.')
                continue
            meta_parsed = meta_parsed.groupdict()
            if meta_info['convert'] is not None:
                for k, v in meta_parsed.items():
                    meta_parsed[k] = meta_info['convert'](v)
            kwargs |= meta_parsed

        print(kwargs)

        return cls.from_csv(tab_file, delimiter=delimiter, header=header, rename_columns=rename_columns, **kwargs)
...
class CSOnline(MeteoData):
    ...
    def update_meteo_data_real_time(self, compute_solar_position=True, compute_clear_sky_irradiance=True,
                                    method_clear_sky_irradiance='ineichen', method_linke_turbidity='ineichen_dlr',
                                    solar_constant=1364):
        """
        ...
        :return:
        """
        cs_data_frame = utils.parse_logger_data(self.url_cs_logger_table, self.tz,
        ...
        if compute_solar_position:
            self.compute_solar_position()
        if compute_clear_sky_irradiance:
            self.compute_clear_sky_irradiance(method_clear_sky_irradiance, method_linke_turbidity, solar_constant)


def _ineichen_linke_turbidity_from_dni(dni, altitude, airmass_abs, solar_constant=1361.1):
    """Computes the Linke turbidity factor using the Ineichen equation for clear-sky DNI and DNI measurements.
    Refer to https://doi.org/10.1016/S0038-092x(02)00045-2 for details on the equations.

    :param dni: (pd.Series) Measured DNI in W/m²
    ...
    :return: linke_turbidity: (pd.Series) Linke turbidity factors soley based on DNI measurements.
    """
    dt = dni.index
    ...
    dni_extra = solar_constant * earth_sun_corr_factor
    linke_turbidity = ((1 / 0.09 * np.log(bcoef * dni_extra / dni)) / airmass_abs + 1)
    return linke_turbidity
...
def linke_turbidity_ineichen_dlr(dni, altitude, sun_el, airmass_abs, solar_constant=1361.1, limit_sun_elevation=20,
    ...
    """
    dt = dni.index
    linke_turbidity = _ineichen_linke_turbidity_from_dni(dni, altitude, airmass_abs, solar_constant)
    ...
    daily_min_turbidity = filtered_turbidity.resample('D').min()
    daily_min_turbidity = daily_min_turbidity.interpolate(method='linear').fillna(method='bfill')
    linke_turbidity = daily_min_turbidity.reindex(linke_turbidity.index, method='ffill')
    return linke_turbidity
...
def get_data_availability_stats(ts_missing, ts_available, idx_name='Data'):
    """
    Returns a DataFrame containing statistics about data availability, comparing missing and available timestamps.

    This function computes various metrics, including the total and fraction of missing data, number of days with
    complete or incomplete data, and daily missing data statistics (median, mean, and standard deviation).

    :param ts_missing: Timestamps corresponding to missing data points.
    :type ts_missing: pandas.DatetimeIndex
    :param ts_available: Timestamps corresponding to available data points.
    :type ts_available: pandas.DatetimeIndex
    :param idx_name: Name for the index of the output DataFrame (default is 'Data').
    :type idx_name: str
    :returns: A pandas DataFrame with the following columns:
        - 'Total data': Number of available data points.
        - 'Total missing': Number of missing data points.
        - 'Fraction missing': Fraction of missing data points relative to total.
        - 'Total days': Total number of unique days in the data.
        - 'Num available days': Number of days with at least one available data point.
        - 'Num incomplete days': Number of days with at least one missing data point.
        - 'Fraction incomplete days': Fraction of days with missing data.
        - 'Median missing per day': Median count of missing data points per day.
        - 'Mean missing per day': Mean count of missing data points per day.
        - 'Std missing per day': Standard deviation of missing data points per day.
    :rtype: pandas.DataFrame
    :raises AssertionError: If ts_missing and ts_available contain overlapping timestamps.
    """
    assert len(ts_missing.intersection(ts_available)) == 0, 'ts_missing and ts_available must not share timestamps'
    ts_all = pd.DatetimeIndex(ts_missing.union(ts_available)).sort_values()
    num_missing = len(ts_missing)
    num_available = len(ts_available)
    num_all = len(ts_all)
    start_date, end_date = ts_all.min().date(), ts_all.max().date()
    full_date_range = pd.date_range(start=start_date, end=end_date, freq='D')
    missing_per_day = pd.Series(ts_missing).groupby(ts_missing.date).count()
    missing_per_day = missing_per_day.reindex(full_date_range, fill_value=0)
    num_days_all = len(np.unique(ts_all.date))
    num_days_available = len(np.unique(ts_available.date))
    num_incomplete_days = len(missing_per_day[missing_per_day > 0])
    df_stats = pd.DataFrame({
    'Total data': num_available,
    'Total missing': num_missing,
    'Fraction missing': num_missing/num_all,
    'Total days': num_days_all,
    'Num available days': num_days_available,
    'Num incomplete days': num_incomplete_days,
    'Fraction incomplete days': num_incomplete_days / num_days_all,
    'Median missing per day': missing_per_day.median(),
    'Mean missing per day': missing_per_day.mean(),
    'Std missing per day': missing_per_day.std()}, index = [idx_name])
    return df_stats
